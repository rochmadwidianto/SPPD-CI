...
/**
* ================= doc ====================
* FILENAME     : PerjalananDinas.php
* @package     : PerjalananDinas
* scope        : PUBLIC
* @Created     : 2018-10-02
* @Modified    : 2018-10-02
...
class PerjalananDinas extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('MLapPerjalananDinas');
        $this->load->library('form_validation');
    }

    public function index()
    {
        $kotatujuan = $this->MLapPerjalananDinas->get_all();

        $data = array(
            'kotatujuan_data' => $kotatujuan
        );

        $this->template->display('lap_perjalanan_dinas/lap_perjalanan_dinas_list', $data);
    }

    public function read($id) 
    {
        $row = $this->MLapPerjalananDinas->get_by_id($id);
        if ($row) {
            $data = array(
        		'kotaTujuanId' => $row->kotaTujuanId,
        		'kotaTujuanNama' => $row->kotaTujuanNama,
	        );
            $this->template->display('lap_perjalanan_dinas/lap_perjalanan_dinas_read', $data);
        } else {
    ...
    public function create() 
    {
        ...
        $this->template->display('lap_perjalanan_dinas/lap_perjalanan_dinas_form', $data);
    }
    ...
    public function create_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->MLapPerjalananDinas->insert($data);
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->MLapPerjalananDinas->update($this->input->post('kotaTujuanId', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->MLapPerjalananDinas->delete($id);
    ...
    public function excel()
    {
        ...
        $namaFile = "lap_perjalanan_dinas.xls";
        $judul = "lap_perjalanan_dinas";
        $tablehead = 0;
        $tablebody = 1;
        $nourut = 1;
        //penulisan header
	...
	foreach ($this->MLapPerjalananDinas->get_all() as $data) {
            $kolombody = 0;
...
/* End of file PerjalananDinas.php */
/* Location: ./application/controllers/PerjalananDinas.php */
