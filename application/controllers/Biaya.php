...
class Biaya extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('MLapBiaya');
        $this->load->library('form_validation');
    }

    public function index()
    {
        $kotatujuan = $this->MLapBiaya->get_all();

        $data = array(
            'kotatujuan_data' => $kotatujuan
        );

        $this->template->display('lap_biaya/lap_biaya_list', $data);
    }

    public function read($id) 
    {
        $row = $this->MLapBiaya->get_by_id($id);
        if ($row) {
            $data = array(
        		'kotaTujuanId' => $row->kotaTujuanId,
        		'kotaTujuanNama' => $row->kotaTujuanNama,
	        );
            $this->template->display('lap_biaya/lap_biaya_read', $data);
        } else {
    ...
    public function create() 
    {
        ...
        $this->template->display('lap_biaya/lap_biaya_form', $data);
    }
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->MLapBiaya->update($this->input->post('kotaTujuanId', TRUE), $data);
	...
	foreach ($this->MLapBiaya->get_all() as $data) {
            $kolombody = 0;
